...
class PatchEmbedWithAdditional(nn.Module):
    """ Image to Patch Embedding
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, norm_layer=None):
        ...
        # self.embed = nn.Parameter(torch.randn(embed_dim, in_chans * patch_size[0] * patch_size[1]))
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()
        self.rearrange = Rearrange('b (h w) (c p1 p2) 1 -> b c (h p1) (w p2)', p1=self.patch_size[0], p2=self.patch_size[1], h=self.patch_grid[0], w=self.patch_grid[1])

        #self.linear = nn.Parameter(torch.randn(self.num_patches, in_chans * patch_size[0] * patch_size[1], in_chans * patch_size[0] * patch_size[1]))
        self.in_chans = in_chans

    def forward(self, x):
        B, T, K, L = x.shape
        # print(x.shape)
        # assert L == 1 and T == self.num_patches and K == self.in_chans * self.patch_size[0] * self.patch_size[1]
        # x = torch.matmul(self.linear, x)
        x = self.rearrange(x)
        #x = torch.matmul(self.embed, x) # not using einops
        #x.squeeze_(-1)
        # print(x.shape)

        #x2 = x.view(B, self.in_chans, self.img_size[0], self.img_size[1])
        #for i in range(B):
        #    x2[i] = rearrange(x[i], '(h w c) (p1 p2) 1 -> c (h p1) (w p2)', p1=self.patch_size[0], p2=self.patch_size[1], h=self.patch_grid[0], w=self.patch_grid[1])
        #    gc.collect()
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if hybrid_backbone is not None:
            ...
            self.patch_embed = PatchEmbedWithAdditional(
